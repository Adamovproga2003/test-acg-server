...
export class ApiService {
  ...
  private async refreshAccessToken(): Promise<string> {
    ...
    try {
      ...
      const response = await lastValueFrom(this.httpService.post(`${this.apiUrl}/refresh`, { refresh_token: refreshToken }));
      const { access_token, refresh_token } = response.data;

      await this.cacheManager.set('access_token', access_token, 3600000);
      await this.cacheManager.set('refresh_token', refresh_token, 7 * 24 * 3600000);
  ...
  private async loginWithCredentials(): Promise<string> {
    try {
      ...
      const response = await lastValueFrom(this.httpService.post(`${this.apiUrl}/token`, {
  ...
  request(method: string, path: string, data?: any): Observable<any> {
    return from(this.getAccessToken()).pipe(
      ...
      catchError((error: AxiosError) => {
        this.logger.error(`Request failed: ${error.message}`, error.response?.data);
        if (error.response && (error.response.status === 401 || error.response.status === 307)) {
          this.logger.debug('Unauthorized or redirect, refreshing token and retrying');
          return from(this.refreshAccessToken()).pipe(
            mergeMap(newToken => this.makeRequest(method, path, data, newToken))
          );
        }
  ...
  private makeRequest(method: string, path: string, data: any, token: string): Observable<AxiosResponse> {
    ...
    return this.httpService.request({
      ...
      tap(response => this.logger.debug(`Request successful: ${response.status}`)),
      catchError((error: AxiosError) => {
        this.logger.error(`Request error: ${error.message}`, error.response?.data);
        return throwError(() => error);
      })
    );
  }
